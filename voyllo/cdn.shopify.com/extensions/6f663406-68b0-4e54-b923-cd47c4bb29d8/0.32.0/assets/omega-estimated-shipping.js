var OtEstimatedRootLink = "https://apps3.omegatheme.com/estimated-shipping";
// var OtEstimatedRootLink = "http://localhost/estimated-shipping";
(function (e, s) {
  e.src = s;
...
function isIEBrowser() {
  ...
  var isme = ua.indexOf("Edge");
  // var estimatedVersion = getVersion();
  var estimatedVersion = new Date().getTime();

  if (typeof estimatedVersion == "undefined" || estimatedVersion == null) {
    estimatedVersion = 100;
  }
  if (
    msie > 0 ||
    ...
    $.getScript(
      OtEstimatedRootLink +
        "/client/estimated-shipping-ie.js?v=" +
        estimatedVersion
    ).done(function () {});
  } else {
    otEstimatedInit();
...
function otEstimatedInit() {
  if (typeof window.OtEstimatedApp == "undefined" || !window.OtEstimatedApp) {
    if (!OtEstimatedRootLink || OtEstimatedRootLink == "") {
      var OtEstimatedRootLink =
        "https://apps3.omegatheme.com/estimated-shipping";
      // var OtEstimatedRootLink = "http://localhost/estimated-shipping";
    }
    var OtEstimatedMainClass = "ot-estimated-shipping";
    var timeBegin = new Date().getTime();
    var currentVariant = document.location.href.split("variant=")[1];

    class OtEstimatedStaticMethods {
      // Because we defined this is a static methods,
      // so all params we will need passing to it
      static loadJquery({ wantedVersion }) {
        wantedVersion = wantedVersion || "1.9.1";
        return new Promise((resolve) => {
          if (
            typeof jQuery === "undefined" ||
            parseFloat(jQuery.fn.jquery) < parseFloat(wantedVersion)
          ) {
    ...
    class OtEstimated {
      ...
      constructor() {
        ...
        this._ip2LocationApi =
          this._rootLink + "/client/services/Ip2Location.php";
        ...
        for (let script of scripts) {
          if (
            ...
            script
              .getAttribute("src")
              .indexOf("estimated-shipping/client/estimated-shipping.js") > -1
          ) {
            me = script.getAttribute("src");
          }
        }
        this._scriptUrl = me;
        // Current version
        ...
        this._product =
          typeof meta != "undefined" && meta.product
            ? meta.product
            : typeof otEstProduct !== "undefined"
            ? otEstProduct
            : typeof __st != "undefined"
            ? { id: __st.rid, variants: [], vendor: "" }
            : {};
        ...
        this._selectors = {
          ...
          shippingMethodSelectBoxClass:
            OtEstimatedMainClass + "-shipping-methods-select-box",
          shippingMethodSelectItemClass:
            OtEstimatedMainClass + "-shipping-methods-select-item",
        ...
        if (!window.otEstAppData) {
          return new Promise((resolve) => {
            ...
            OtEstimatedStaticMethods.getJsonFile({
              ...
              OtEstimatedStaticMethods.logTime(
                `JSON data loaded after : {diff} milliseconds`
              );
              resolve();
            });
          });
        } else {
          this._data = window.otEstAppData.data.app;
          this._generalSettings = window.otEstAppData.data.app.settings;
        }
      }
      ...
      _validateCondition() {
        return new Promise((resolve, reject) => {
          if (
            this._generalSettings.enable_app == 1 &&
            this._generalSettings.show_on_pages
              .split(",")
              .some((e) => this._currentUrl.indexOf(e) > -1)
          ) {
      ...
      _validateProducts() {
        return new Promise((resolve, reject) => {
          ...
          window.otCurrentVariantApp = currentVariant;
          if (this._generalSettings.disable_when_product_is_out_of_stock == 1) {
            if (
              typeof window.OtForceShowAppOutOfStock !== "undefined" &&
              !!window.OtForceShowAppOutOfStock
            ) {
              resolve();
            }
            this._getProductById({
              ...
              window.otProductInfo = response.product;
              const productVariant = response.product.variants;
              if (this._product && Array.isArray(this._product.variants)) {
                if (typeof variantID != "undefined") {
                  if (
                    ...
                    $(".ot-estimated-shipping").empty();
                    reject();
                  }
                } else {
                  // variantID = this._product.variants[0]['id'];
                  if (
                    productVariant.every(
            ...
            const changeVariant = setInterval(handleChangeVariant, 1500);
            function handleChangeVariant() {
              const newVariant = document.location.href.split("variant=")[1];
              ...
              if (
                ...
                currentVariant = newVariant;
                window.OtEstimatedApp = new OtEstimated();
              }
            }
          } else {
            resolve();
      ...
      _switchToOptionalLayout() {
        var customLayout = this._jQuery(`.${this._selectors.mainClass}`).attr(
          "data-layout"
        );
        if (
          typeof customLayout !== typeof undefined &&
          customLayout !== false
        ) {
        ...
        return new Promise((resolve) => {
          if (this._generalSettings.date_locale === "en") {
            ...
            OtEstimatedStaticMethods.getScript({
              ...
              OtEstimatedStaticMethods.logTime(
                "Localized after : {diff} milliseconds"
              );
              resolve();
            });
          }
      ...
      _createParentClass() {
        ...
        let { custom_position, position, render_method } =
          this._generalSettings;
        const jQueryFunc = this._jQuery;
        let extendTo = custom_position || position;
        const mainAppDiv = `<!-- Start OmegaTheme Estimated Shipping For Shopify -->
                        <div class='${this._selectors.mainClass}'></div>
                        <!-- End OmegaTheme Estimated Shipping For Shopify -->`;
        if (window.otEstAppData != undefined) {
          let timeout = 0;
          const renderInterval = setInterval(() => {
            timeout += 100;
            const DOMSelector = this._jQuery(extendTo);
            const mainAppDiv = this._jQuery(`.${this._selectors.mainClass}`);
            if (DOMSelector.length && mainAppDiv.length == 0) {
              renderMainApp();
              this._initLayout();
              this._applyCss();
              clearInterval(renderInterval);
            } else if (mainAppDiv.length) {
              this._initLayout();
              this._applyCss();
              clearInterval(renderInterval);
        ...
        function renderMainApp() {
          switch (render_method) {
            case "after":
              jQueryFunc(extendTo).after(mainAppDiv);
              break;
            case "before":
              jQueryFunc(extendTo).before(mainAppDiv);
              break;
            case "prepend":
              jQueryFunc(extendTo).prepend(mainAppDiv);
              break;
            default:
              jQueryFunc(extendTo).append(mainAppDiv);
              break;
          }
        }
        // }
      ...
      _applyCss() {
        ...
        if (
          ...
          Shopify.shop != "project-nursery.myshopify.com" &&
          Shopify.shop != "pilk-india.myshopify.com" &&
          Shopify.shop != "the-amyra-store.myshopify.com"
        ) {
          htmlcss += `<link href='${this._rootLink}/assets/lib/font-awesome/css/font-awesome.min.css?v=${this._v}' rel='stylesheet'>`;
        }

        htmlcss += `
            <link href='${this._rootLink}/assets/css/estimated-shipping.css?v=${
          this._v
        }' rel='stylesheet' type='text/css'>
            <link href='${this._rootLink}/assets/flags/flags.css?v=${
          this._v
        }' rel='stylesheet' type='text/css'>
            <style>
                ...
                [class^="${this._selectors.mainClass}-basic-layout"] {
                    ...
                    border-radius     : 5px;
                    background-color :  ${
                      this._generalSettings.background_color
                    };
                    padding : 10px;
                    margin-top : 10px;
                }
                ...
                ${
                  this._generalSettings.choose_layout == "card"
      ...
      _calculateStartAndEndDay({ minimumDays, estimatedDays }) {
        var self = this;
        var workingDays =
          this._generalSettings.week_working_days !== ""
            ? JSON.parse(this._generalSettings.week_working_days)
            : [];
        var specialDayOffs =
          this._generalSettings.specific_day_off &&
          this._generalSettings.specific_day_off != "" &&
          this._generalSettings.specific_day_off != null
            ? JSON.parse(this._generalSettings.specific_day_off)
            : [];
        ...
        function calculateStartDay(minimumDays, specialDayOffs, workingDays) {
          ...
          var isCutOff = checkCutOff(startDay, workingDays, specialDayOffs);
          if (isCutOff) {
            minimumDays++;
          }
          ...
          function checkCutOff(startDay, workingDays, specialDayOffs) {
            const formatStartDay = handleLocalDateTime(startDay);
            var today = startDay.getDay();
      ...
      _generateMainEstimatedText({
        ...
        var self = this;
        const startDayCountDown = self._createDate();
        var today = startDayCountDown.getDay();
        const currentHour = startDay.getHours();
        const currentMinute = startDay.getMinutes();
        const weekDayCountDown = window.workingDays.find((e) => e.day == today);
        const cutOffHour = weekDayCountDown.cut_off_after.split(":")[0];
        const cutOffMinute = weekDayCountDown.cut_off_after.split(":")[1];
        const isAppendCountDown =
          currentHour < cutOffHour ||
          (currentHour == cutOffHour && currentMinute < cutOffMinute);
        if (
          self._generalSettings.textCountDownFormat != "" &&
          self._generalSettings.textCountDownFormat != null &&
          isAppendCountDown
        ) {
        ...
        icon = icon
          ? icon
          : self._generalSettings.estimated_icon !==
            '<i class="fa fa-check"></i>'
          ? self._generalSettings.estimated_icon
          : "";
        if (
          icon.includes("{country_code}") &&
          self._userInfo &&
          self._userInfo.country_code
        ) {
          icon = icon.replace(
            "{country_code}",
            self._userInfo.country_code.toLowerCase()
          );
        }

        var dateFormat = self._generalSettings.date_format;
        // var estimatedText = ''
        // const checkEstTextContainDate =
        //   rawEstimatedText.includes("{date}") ||
        //   rawEstimatedText.includes("{date + 1}");
        // if (checkEstTextContainDate) {

        // }
        var estimatedText = `<span class='${
          self._selectors.mainTextClass
        }-estimated'>
            ${rawEstimatedText.replace(
              "{date}",
              `<span class="${self._selectors.startDayTextClass}">${moment(
                startDay
        ...
        estimatedText = estimatedText.replace(
          "{date+1}",
          `<span class="${self._selectors.endDayTextClass}">${moment(
            endDay
          ).format(dateFormat)}</span>`
        );
        if (
          ...
          var weekDay = workingDays.find((e) => e.day == today);
          if (weekDay.enable != "0" && isAppendCountDown) {
            self._generalSettings.textCountDownFormat = `
                    <span class="${self._selectors.mainTextClass}-countdown">
                    ${self._generalSettings.textCountDownFormat.replace(
                      "{TimeCountDown}",
                      `<span class="otCountDown"></span>`
                    )}</span>`;
        ...
        function appendCountDownTime(generalSettings, workingDays) {
          ...
          var weekDay = workingDays.find((e) => e.day == today);
          if (weekDay.enable != "0") {
            let caculatorTime = diff(
              currentHour + ":" + currentMinute + ":" + currentSecond,
              weekDay.cut_off_after + ":" + "59"
            );
        ...
        function diff(start, end) {
          ...
          if (Shopify.shop != "sportmbm.myshopify.com") {
      ...
      _bindEstimatedTextAsHiddenInput({ rawEstimatedText, startDay, endDay }) {
        ...
        if (
          this._currentUrl.indexOf("products") > -1 &&
          this._generalSettings.show_on_line_item == 1 &&
          rawEstimatedText
        ) {
          // Get Date Format
          ...
          window.otEstimatedText = rawEstimatedText.replace(/<[^>]+>/g, "");
          rawEstimatedText = rawEstimatedText.replace(/<[^>]+>/g, "");
          ...
          //debugger;
          if (typeof window.typePropertiesLableInput == "undefined")
            window.typePropertiesLableInput = "Estimated Shipping";
          if (raw.length > 0) {
            ...
            if (raw.length >= 1) {
              ...
              if (startWithStartDate) {
                ...
                fieldValue =
                  ...
                  " " +
                  raw[1].replace("{date+1}", moment(endDay).format(dateFormat));
            ...
            if (fieldName == "") {
              if (
                typeof this._generalSettings.labelCheckout != "undefined" &&
                this._generalSettings.labelCheckout != ""
              ) {
                var fieldName = this._generalSettings.labelCheckout;
              } else {
                var fieldName =
                  Shopify.shop == "dream-bride-boutique-retail.myshopify.com"
                    ? "Estimated Shipping date is"
                    : "Estimated between";
              }
              if (Shopify.shop == "dream-bride-boutique-retail.myshopify.com") {
                fieldName = "Estimated Shipping date is";
              }
            }
            // Toggle an hidden input to form
            if (typeof window.propertiesLableInput == "undefined")
              window.propertiesLableInput = "";
            if (
              this._jQuery(
                `form[action="/cart/add"] input[name="properties[${fieldName}]"]`
              ).length
            ) {
              this._jQuery(
                `form[action="/cart/add"] input[name="properties[${fieldName}]"]`
            ...
            this._jQuery('form[action^="/cart/add"]').prepend(`
                        <input type="hidden" class="otExistRule"  name="properties[${fieldName}]" value="${rawEstimatedText.replace(
              /<[^>]+>/g,
              ""
            )}">
                    `);
      ...
      _displayShippingMethods(shippingMethods, app, currentMethod, settings) {
        ...
        if (settings.choose_layout == "select") {
          ...
          if (
            ...
            if (
              this._generalSettings.textCountDownFormat != "" &&
              this._generalSettings.textCountDownFormat != null
            ) {
              const workingDays = JSON.parse(
                this._generalSettings.week_working_days
              );
              var todayCountdown = new Date().getDay();
              var weekDay = workingDays.find((e) => e.day == todayCountdown);
              if (weekDay.enable != "0") {
                this._generalSettings.textCountDownFormat =
                  this._generalSettings.textCountDownFormat.replace(
                    "{TimeCountDown}",
                    `<span class="otCountDown"></span>`
                  );
              }
            ...
            selectMethods += `<h4>Shipping method</h4>`;
            window.propertiesLableInput = shippingMethods[0]["name"] + " - ";
            window.typePropertiesLableInput = "Method shipping";
            const estimatedSettings = window.OtEstimatedDate.settings;
            let privacyLink = "";
            if (estimatedSettings.privacy_page_url != "") {
              privacyLink += `
            ...
            for (var method of shippingMethods) {
              ...
              const todayCard = new Date().getDay();
              const weekDayCountDown = workingDays.find(
                (e) => e.day == todayCard
              );
              const cutOffHour = weekDayCountDown.cut_off_after.split(":")[0];
              const cutOffMinute = weekDayCountDown.cut_off_after.split(":")[1];
              ...
              selectMethods += `<div class="${
                this._selectors.itemMethodShipping
              } ${this._selectors.itemMethodShipping}-${
                method.id
              }" ><input type="radio" id="${
                method.id
              }" name="check_shipping_method" value="${
                method.id
              }"   class="ot-estimated-shipping-item-radio ${
                this._selectors.shippingMethodSelectItemClass
              }">
                        <label for=${method.id}></label>
                        <div class="ot-estimated-shipping-title-shipping">${
                          method.icon
                        } ${method.name}</div>
                        <div class="ot-estimated-shipping-text-shipping">${estimatedText}</div>
                        <div class="ot-estimated-shipping-text-countdown">${
                          method.showCountdown === "1" && isAppendCountDown
                            ? this._generalSettings.textCountDownFormat
                            : ""
                        }</div>
            ...
            jQuery(document).on(
              ...
              () => {
                ...
                var methodUnChecked = jQuery(
                  "input[name=check_shipping_method]"
                );

                for (let i = 0; i < methodUnChecked.length; i++) {
                  if (!methodUnChecked[i].checked) {
                    methodUnChecked[
                      i
          ...
          jQuery(".ot-estimated-shipping-item-shipping-method").on(
            "click",
            function () {
              ...
              divParent = divParent[0].children;
              for (let i = 1; i < divParent.length; i++) {
                divParent[i].style.color = `${settings.text_color}`;
              }
      ...
      _getUserInfoByIp(forceGet) {
        return new Promise((resolve) => {
          if (
            forceGet ||
            (this._generalSettings.get_customer_zipcode &&
              Number(this._generalSettings.get_customer_zipcode) === 1 &&
              Number(this._generalSettings.layout) != 1)
          ) {
            this._jQuery
              .ajax({
              ...
              .done((result) => {
                OtEstimatedStaticMethods.logTime(
                  "Get user info after: {diff} milliseconds"
                );
      ...
      _createDate(d) {
        ...
        return moment(date).utcOffset(-diff)._d;
      }
    }
    ...
    class OtEstimatedDate {
      constructor({
        ...
        this.selectors = {
          ...
          shippingPrivacyClass:
            OtEstimatedMainClass + "-basic-shipping-privacy",
          shippingPrivacyTextClass:
            OtEstimatedMainClass + "-basic-shipping-privacy-text",
          shippingPrivacyMoreClass:
            OtEstimatedMainClass + "-basic-shipping-privacy-more",
          shippingPrivacyMorePopupClass:
            OtEstimatedMainClass + "-basic-shipping-privacy-more-popup",
      ...
      init() {
        this.findSpecificTargets()
          ...
          .then(() => {
            window.otAvailableShippingMethods = this.availableShippingMethods;
      ...
      findSpecificTargets() {
        return new Promise((resolve) => {
          ...
          this._app._getCollectionsByProductId().then((productCollections) => {
            productCollections = productCollections || [];
            this.specificTargets = allTargets.filter((target) => {
              return (
                Number(target.value) === productId ||
                productCollections.indexOf(Number(target.value)) > -1 ||
                target.value.toUpperCase() === productVendor.toUpperCase()
              );
            });
            resolve();
          });
        ...
        return new Promise((resolve) => {
          if (this.specificTargets.length) {
            this.availableRules = this.specificRules.filter((rule) => {
              return this.specificTargets.some(
                (target) => Number(target.rule_id) === Number(rule.id)
              );
            });
          }
          resolve();
          ...
          if (this.availableRules.length) {
            ...
            if (this.specificRuleTargets.length > 0) {
              ...
              this.availableRules.forEach(function (element, index) {
                self.specificRuleTargets.forEach(function (
                  specificRule,
                  indexspecificRule
                ) {
              ...
              this.availableRules = [];
              this.availableRules = sortAvailableRulesP.concat(
                sortAvailableRulesC,
                sortAvailableRulesV
              );
            }
            this.shippingMethods.forEach((method) => {
              ...
              this.availableRules.forEach(function (element) {
                if (Object.keys(rule).length === 0) {
                  if (
                    Number(method.id) === Number(element.shipping_method_id)
                  ) {
                    rule = element;
                  }
                }
              ...
              if (Object.keys(rule).length > 0) {
                availableMethods.push({
                  ...
                  privacy_text: method.privacy_text
                    ? method.privacy_text.replaceAll("\n", "<br>")
                    : "",
                  rule_id: rule.id,
                  ...
                  custom_text: rule.custom_text
                    ...
                    : "",
                  showCountdown: method.showCountdown,
                });
              }
            });
            this.availableShippingMethods = availableMethods;
          } else if (Number(this.settings.only_show_specific_targets) === 0) {
            this.availableShippingMethods = this.shippingMethods.filter(
              (method) => Number(method.is_private) === 0
            );
          } else {
            this.availableShippingMethods = [];
          }
      ...
      displayEstimatedTime() {
        ...
        var mainText = this._app._generateMainEstimatedText({
          ...
          startDay,
          endDay,
          showCountdown: this.currentMethod.showCountdown,
        });
        listHtml.push(mainText);

        // Shipping privacy
        var privacyText = this.currentMethod.privacy_text.replaceAll(
          "\n",
          "<br>"
        );
        if (privacyText != "") {
          privacyText = privacyText.replace(
            "{date}",
            this.currentMethod.estimated_days
          );
          var shippingPrivacy = "";
          ...
          if (this.settings.privacy_more) {
            shippingPrivacy += `
                        <span class="${
                          this.selectors.shippingPrivacyMoreClass
                        }">
                            <i class="fas fa-info-circle" aria-hidden="true"></i>
                            <span class="${
                              this.selectors.shippingPrivacyMorePopupClass
                            }" style="display: none;">
        ...
        if (this.currentMethod.custom_text) {
          listHtml.push(
            `<p class="${
              this.selectors.productCustomTextClass
            }">${this.currentMethod.custom_text.replaceAll("\n", "<br>")}</p>`
          );
        }

        // Display
        if (jQuery(`.${this.selectors.basicLayoutClass}`).length == 0) {
          jQuery(`.${this.selectors.mainClass}`).append(
            `<div class="${this.selectors.basicLayoutClass}"></div>`
          );
        }
        jQuery(`.${this.selectors.basicLayoutClass}`).empty();
        listHtml.forEach((info) =>
          jQuery(`.${this.selectors.basicLayoutClass}`).append(info)
        );
        jQuery(`.${this.selectors.shippingPrivacyMoreClass}`).on(
          "click",
          () => {
            jQuery(
              `.${this.selectors.shippingPrivacyMoreClass} .${this.selectors.shippingPrivacyMorePopupClass}`
            ).toggle();
          }
        );
          ...
          (e, context) => {
            ...
            this.availableShippingMethods.forEach((method, index) => {
              if (method.id == methodId) {
                ...
                if (jQuery(".otEstimatedShipping").length > 0)
                  jQuery(".otEstimatedShipping").remove();
                window.propertiesLableInput = method.name + " - ";
                window.typePropertiesLableInput = "Method shipping";
                if (self.generalSettings.showNameMethodInPropertites == 1) {
                  jQuery('form[action^="/cart/add"]').prepend(
                    ` <input type="hidden" class="otEstimatedShipping" name="properties[${window.typePropertiesLableInput}]" value="${window.propertiesLableInput}"> `
                  );
                }
              }
    ...
    class OtEstimatedZipcode {
      constructor({ _app, settings }) {
        ...
        this.selectors = {
          ...
          zipcodeNotAvailableClass:
            OtEstimatedMainClass + "-zipcode-not-available",
          codAvailableClass: OtEstimatedMainClass + "-zipcode-cod-available",
          codNotAvailableClass:
            OtEstimatedMainClass + "-zipcode-cod-not-available",
          ...
          zipcodeShowInputZipcodeFormBtnClass:
            OtEstimatedMainClass + "-zipcode-show-input-zipcode-form",
      ...
      init() {
        if (this._app._generalSettings.requireAddToCart == 1) {
            ...
            .then(() => {
              if (this.userZipcode) {
                ...
                if (this.currentZipcode) {
                  if (this._app._generalSettings.requireAddToCart == 1) {
                    jQuery(this.selectors.addToCartButton).attr(
                      "disabled",
                      false
                    );
                  }
                ...
                if (this._app._generalSettings.get_customer_zipcode == "0") {
                  this.displayInputZipcodeForm();
                }
              }
            });
      ...
      getDataZipcode() {
        if (!window.otEstAppData) {
          ...
          return new Promise((resolve) => {
            var self = this;
            const zipcodeData = window.otEstAppData.dataZipcode.app.zipcode;
            self.listZipcodes = zipcodeData.listZipcodes;
            self.specificProducts = zipcodeData.specificProducts;
            self.specificCollections = zipcodeData.specificCollections;
            self.specificVendors = zipcodeData.specificVendors;
            resolve();
          });
        }
      }
      findAvailableZipcodes() {
        return new Promise((resolve, reject) => {
          this.findSpecificZipcodes().then(() => {
            if (this.isUseSpecificZipcodes) {
              if (this.specificZipcodes.length) {
                this.availableZipcodes = this.specificZipcodes.map(
                  (specificZipcode) => {
                    let rawZipcodes = this.listZipcodes.find(
                      (c) => c.id == specificZipcode.zipcode_id
                    );
                    specificZipcode.custom_text =
                      specificZipcode.custom_text.replaceAll("\n", "<br>");
                    rawZipcodes.minimum_days = specificZipcode.minimum_days;
                    rawZipcodes.estimated_days = specificZipcode.estimated_days;
                    const newSpecificZipcode = {
                      ...rawZipcodes,
                      ...specificZipcode,
                    };
                    return newSpecificZipcode;
                  }
                );
              }
            } else {
        ...
        return new Promise((resolve) => {
          if (this._app._generalSettings.get_customer_zipcode == "0") {
            ...
            if (!this._app._userInfo || this._app._userInfo == null) {
              this._app._getUserInfoByIp(true).then(() => {
        ...
        return new Promise((resolve) => {
          ...
          switch (this.settings.for_all_product) {
            case "0":
              ...
              this.specificZipcodes = this.specificProducts.filter(
            ...
            case "2":
              // Specific vendors
              this.isUseSpecificZipcodes = true;
              this.specificZipcodes = this.specificVendors.filter(
                (e) => e.name.toUpperCase() == productVendor.toUpperCase()
              );
              resolve();
              break;
            case "3":
              ...
              this._app._getCollectionsByProductId().then((listCollections) => {
                if (listCollections && Array.isArray(listCollections)) {
                  this.specificZipcodes = this.specificCollections.filter(
                    (specificCollection) =>
                      listCollections.some(
                        (collectionId) =>
                          collectionId ===
                          Number(specificCollection.collection_id)
                      )
                  );
                  resolve();
                } else {
      ...
      displayInputZipcodeForm() {
        ...
        if (jQuery(`.${this.selectors.zipcodeLayoutClass}`).length == 0) {
          jQuery(`.${this.selectors.mainClass}`).append(
            `<div class="${this.selectors.zipcodeLayoutClass}"></div>`
          );
        }
        jQuery(`.${this.selectors.zipcodeLayoutClass}`).empty();
        jQuery(`.${this.selectors.zipcodeLayoutClass}`).append(`
                <div class="${this.selectors.zipcodeSubmitBtnClass}-title">${
          this.settings.input_label
        }</div>
                <br>
                <div class="${this.selectors.zipcodeSubmitBtnClass}-input-form">
                  <input style="display:inline-block;" class="${
                    this.selectors.zipcodeInputClass
                  }" placeholder="${this.settings.input_placeholder.replaceAll(
          '"',
          "'"
        )}">
                <p class="${this.selectors.zipcodeSubmitBtnClass}"
                    style="
                        background-color: ${
                          this.settings.submit_button_background_color
                        };
                        cursor: pointer;
                        padding: 5px 10px;
                        color: #fff;
            ...
            `);
        jQuery(
          `body .${this.selectors.mainClass} .${this.selectors.zipcodeLayoutClass} .${this.selectors.zipcodeSubmitBtnClass}`
        ).on("click", () => {
          let inputVal = jQuery(
            `.${this.selectors.mainClass} .${this.selectors.zipcodeLayoutClass} .${this.selectors.zipcodeInputClass}`
          )
            .val()
            .replace(/<[^>]*>/g, "");
          if (inputVal) {
            ...
            if (this.currentZipcode) {
              ...
              jQuery(`.${this.selectors.zipcodeNoticeClass}`).html(
                `${this.settings.zipcode_not_available_text} ${inputVal}`
              );
            }
          }
        });
          ...
          return listZipcodes.some((zipcode) => {
            ...
            if (zipcode.includes(":")) {
              ...
              if (
                !isNaN(startZip) &&
                !isNaN(endZip) &&
                Number(startZip) <= Number(specificZipcode) &&
                Number(specificZipcode) <= Number(endZip)
              ) {
            ...
            // Second: zipcode starting with user input
              ...
              if (zipcode.toLowerCase() === specificZipcode.toLowerCase()) {
                ...
                specificZipcode
                  .toLowerCase()
                  .startsWith(zipcode.toLowerCase()) &&
                zipcode !== ""
              ) {
                setting["user_zipcode"] = specificZipcode;
                return true;
      ...
      displayZipcodeInfo() {
        ...
        if (zipcodeSetting.show_cash_delivery == 1) {
          ...
          if (zipcodeSetting.cash_delivery_available == 1) {
            listInfo.push(
              `<p class="${this.selectors.codAvailableClass}"><i class="fa fa-check"></i> ${this.settings.cash_available_text}</p>`
            );
          } else {
            listInfo.push(
              `<p class="${this.selectors.codNotAvailableClass}"><i class="fa fa-times"></i> ${this.settings.cash_not_available_text}</p>`
            );
          }
        }
        ...
        if (zipcodeSetting.show_extra_cost == 1) {
          listInfo.push(
            `<p class="${this.selectors.extraCostClass}"><i class="fa fa-check"></i> ${this.settings.extra_cost_text}: ${zipcodeSetting.extra_cost}</p>`
          );
        }
        ...
        if (zipcodeSetting.show_custom_text == 1) {
          listInfo.push(
            `<p class="${
              this.selectors.customTextClass
            }">${zipcodeSetting.custom_text.replaceAll("\n", "<br>")}</p>`
          );
        }
        ...
        listInfo.forEach((info) =>
          jQuery(`.${this.selectors.zipcodeLayoutClass}`).append(info)
        );

        // Bind event when user click on refresh icon
        jQuery(
          `body .${this.selectors.mainClass} .${this.selectors.zipcodeLayoutClass} .${this.selectors.zipcodeShowInputZipcodeFormBtnClass}`
        ).on("click", () => {
          this.displayInputZipcodeForm();
        });
      }
    ...
    class OtEstimatedCountry {
      constructor({
        ...
        this.selectors = {
          ...
          showSelectBoxBtnClass:
            OtEstimatedMainClass + "-country-show-select-box-btn",
          selectBoxClass: OtEstimatedMainClass + "-country-select-box",
          selectCountryClass:
            OtEstimatedMainClass + "-country-select-box-country",
          selectProvinceClass:
            OtEstimatedMainClass + "-country-select-box-province",
        };
      ...
      findAvailableCountries() {
        ...
        if (Array.isArray(this.specificProducts)) {
          listSpecificProductRules = this.specificProducts.filter(
            (specificProduct) => {
              return specificProduct.product_id == productId;
            }
          );
        }

        if (Array.isArray(this.specificCollections)) {
          listSpecificCollectionRules = [];
          this.specificCollections.forEach((specificCollection) => {
            if (
              this.collectionIdsOfCurrentProduct.indexOf(
                +specificCollection.collection_id
              ) >= 0
            ) {
              listSpecificCollectionRules.push(specificCollection);
            }
          });
        }
        ...
        if (listSpecificRules.length > 0) {
          this.availableCountries = listSpecificRules.map((rule) => {
            let country = this.listCountries.find(
              (countryItem) => countryItem.id == rule.country_id
            );
            country.minimum_days = rule.minimum_days;
            country.estimated_days = rule.estimated_days;
            country.custom_text = rule.custom_text.replaceAll("\n", "<br>");
            const newRule = { ...rule, ...country };

            return newRule;
          });
          this.availableCountries.sort(
            (country1, country2) =>
              Number(country1.Ordering) - Number(country2.Ordering)
          );
        } else {
      ...
      getUserCountryCode() {
        return new Promise((resolve) => {
          if (this._app._generalSettings.get_customer_zipcode == 1) {
            if (!this._app._userInfo || this._app._userInfo == null) {
              this._app._getUserInfoByIp(true).then(() => {
      ...
      findUserCountry() {
        ...
        if (this._app._generalSettings.get_customer_zipcode == "0") {
          ...
          let country = this.listCountries.find(
            (e) => e.code.toUpperCase() == this.userCountryCode.toUpperCase()
          );
          if (country) {
      ...
      displayCountryInfo() {
        ...
        var minimumDays =
          displayProvince &&
          displayProvince.hasOwnProperty("use_custom_day") &&
          displayProvince.use_custom_day
            ? displayProvince.minimum_days
            : displayCountry.minimum_days;
        var estimatedDays =
          displayProvince &&
          displayProvince.hasOwnProperty("use_custom_day") &&
          displayProvince.use_custom_day
            ? displayProvince.estimated_days
            : displayCountry.estimated_days;
        ...
        if (displayProvince && displayProvince != null) {
          var deliveryTo = `<p class="${this.selectors.countryTextClass}">${
            displayCountry.delivery_label
          } ${`${displayProvince.name}, `}${displayCountry.name}`;
        ...
        // Show Estimated date
        if (
          typeof displayCountry.custom_text != "undefined" &&
          displayCountry.custom_text != ""
        ) {
          const tempEstText = displayCountry.estimated_text;
          const customText = displayCountry.custom_text.replaceAll(
            "\n",
            "<br>"
          );
          ...
          displayCountry.custom_text = "";
          displayCountry.estimated_text =
            `<span class="estimated-shipping-estimated_text">${tempEstText}</span>` +
            "<br>" +
            `<span class="estimated-shipping-custom-text">${customText}</span>`;
        }
        var mainText = this._app._generateMainEstimatedText({
        ...
        if (displayCountry.display_courier_info == "1") {
          var courier =
            displayCountry.courier_url != ""
              ? ` <a target="_blank" href="${displayCountry.courier_url}">${displayCountry.courier_name}</a>`
              : displayCountry.courier_name;
          var shippingVia =
            '<p class="' +
            this.selectors.courierTextClass +
            '"><i class="fa fa-check"></i> ' +
            displayCountry.courier_label +
            courier +
            "</p>";
        ...
        if (
          displayProvince &&
          displayProvince.hasOwnProperty("custom_info") &&
          displayProvince.custom_info != ""
        ) {
          var customProvinceInfo = `<p class="${
            this.selectors.customInfoClass
          }">${displayProvince.custom_info.replaceAll("\n", "<br>")}</p>`;
          listInfo.push(customProvinceInfo);
        } else if (
          displayCountry.custom_info &&
          displayCountry.custom_info != ""
        ) {
          var customCountryInfo = `<p class="${
            this.selectors.customInfoClass
          }">${displayCountry.custom_info.replaceAll("\n", "<br>")}</p>`;
        ...
        if (jQuery(`.${this.selectors.countryLayoutClass}`).length == 0) {
          jQuery(`.${this.selectors.mainClass}`).append(
            `<div class="${this.selectors.countryLayoutClass}"></div>`
          );
        }
        jQuery(`.${this.selectors.countryLayoutClass}`).empty();
        listInfo.forEach((info) =>
          jQuery(`.${this.selectors.countryLayoutClass}`).append(info)
        );

        jQuery(
          `.${this.selectors.mainClass} .${this.selectors.countryLayoutClass} .${this.selectors.showSelectBoxBtnClass}`
        ).on("click", () => {
      ...
      displaySelectCountry() {
        ...
        if (
          !jQuery(
            `.${this.selectors.mainClass} .${this.selectors.countryLayoutClass} .${this.selectors.selectBoxClass}`
          )[0]
        ) {
          jQuery(`.${this.selectors.mainClass}`).prepend(`
                ...
                `);
          this.availableCountries.forEach((country, index) => {
      ...
      selectCountryChange() {
        ...
        var currentCountry = this.availableCountries.find(
          (e, index) => e.code + "-" + index == currentCode
        );
        ...
        if (
          ...
          currentCountry.show_sub_regions == 1
        ) {
          if (
            !jQuery(
              `.${this.selectors.selectBoxClass} .${this.selectors.selectProvinceClass}`
            )[0]
          ) {
            ...
            currentCountry.provinces.forEach((province) => {
              if (
                !province.hasOwnProperty("enable_shipping") ||
                province.enable_shipping == 1
              ) {
      ...
      bindTrigger() {
        jQuery(
          `.${this.selectors.mainClass} .${this.selectors.selectBoxClass} .${this.selectors.selectCountryClass}`
        ).on("change", () => {
        ...
        jQuery(
          `.${this.selectors.mainClass} .${this.selectors.selectBoxClass} .${this.selectors.selectProvinceClass}`
        ).on("change", () => {
          if (this.userCountry && this.userCountry.provinces.length > 0) {
            var currentProvinceCode = jQuery(
              `.${this.selectors.selectProvinceClass}`
            ).val();
            if (currentProvinceCode) {
              this.userCountryProvince =
                this.userCountry.provinces[
                  this.userCountry.provinces.findIndex(
                    (e) => e.code == currentProvinceCode
                  )
                ];
              this.displayCountryInfo();
            } else {
